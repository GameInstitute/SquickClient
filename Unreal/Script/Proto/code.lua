...
enum ServerMsgId {
	...
	REQ_PVP_GAME_INIT_FINISHED             = 54; // PVP服务器初始化数据完成
	ACK_PVP_GAME_INIT_FINISHED             = 55; 

	REQ_PLAYER_INFO                        = 56; // 加入玩家，
	ACK_PLAYER_INFO                        = 57; // 
	ACK_NEW_PLAYER                         = 58; //
...
LogUnLua: 不存在该Msg, msg id: 202*/
	...
	REQ_PVP_GAME_JOIN                    = 1034; // 请求加入对战中的游戏
	ACK_PVP_GAME_JOIN                    = 1035; 

	REQ_PVP_GAME_QUIT                    = 1036; // 玩家请求退出当前对局，由 Player -> Proxy -> Game -> PvpManager -> Pvp
	ACK_PVP_GAME_QUIT                    = 1037;
...
// 发送每个玩家的数据
message AckPvpGameInit {
}


// 初始化数据完成，广播所有玩家加入游戏
message ReqPvpGameInitFinished {
    int32 code = 1; // PVP服务器状态, 0 正常, 1 不正常
    bytes instance_id = 2;  // instance id
    bytes instance_key = 3; 
    int32 room_id = 4;
    bytes ip = 5;   // PVP服务器IP
	int32 port = 6; // PVP服务器端口
    bytes name = 7; // PVP服务器名称
}

message AckPvpGameInitFinished {
    int32 code = 1;
}
...
message RoomDetails {
	...
	int32 max_players = 6; // 最多人数
	Ident owner = 7;       // 房主
	repeated RoomPlayer players = 8; // 房间内所有玩家
	int32 map_id = 9;           // 地图ID
	PvpServer server = 10;       // pvp server信息
}
...
// 请求开始游戏
message ReqStartPvpGame {
	int32 room_id = 1;
}

message AckStartPvpGame {
	int32 code = 1;
}

message ReqPvpGameQuit {
	int32 room_id = 1;
}

message AckPvpGameQuit {
	int32 code = 1;
}
]]